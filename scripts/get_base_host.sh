...
remote_host=$($here/get_ansible_var.sh remote_addr localhost)

# echo wired is $wired_host
# echo wireless is $wireless_host

if ping -q -t 1 -c 2 $wired_host >& /dev/null;
then
    echo wired_base
elif ping -q -t 1 -c 2 $wireless_host >& /dev/null;
then
    echo wireless_base
elif ping -q -t 1 -c 2 $remote_host >& /dev/null;
then
    echo remote_base
else
